...
Description: This script takes a SAM file and outputs a properly formatted SAM file that
has removed all PCR duplicates. The first time a unique read is seen, it will be written to 
the output file. Subsequent versions of the same read will not be saved. The user must provide
a list of UMIs formatted in the same way as STL96.txt in this repository. Alignments with UMIs
that are not in submitted list are considered illigitimate and will not be considered. The input
SAM file must have already been sorted by chromosome with samtools sort. The input SAM file must
also have .sam in its name.
'''
...
print("Given {} SAM file alignments, {} were identified as PCR duplicates".format(total_ct, dup_ct))
